...
(deftype IndexedSeq [arr i]
  ...
  (-empty [coll] js/cljs.core.List.EMPTY)
...
(set! js/cljs.core.List.EMPTY (EmptyList. nil))
...
(deftype PersistentVector [meta cnt shift root tail ^:mutable __hash]
  ...
  (-pop [coll]
    (cond
     ...
     (== 1 cnt) (-with-meta js/cljs.core.PersistentVector.EMPTY meta)
     (< 1 (- cnt (tail-off coll)))
      (PersistentVector. meta (dec cnt) shift root (.slice tail 0 -1) nil)
      :else (let [new-tail (unchecked-array-for coll (- cnt 2))
                  nr (pop-tail coll shift root)
                  new-root (if (nil? nr) js/cljs.core.PersistentVector.EMPTY_NODE nr)
                  cnt-1 (dec cnt)]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta js/cljs.core.PersistentVector.EMPTY meta))
...
(set! js/cljs.core.PersistentVector.EMPTY_NODE (VectorNode. nil (make-array 32)))

(set! js/cljs.core.PersistentVector.EMPTY
  (PersistentVector. nil 0 5 js/cljs.core.PersistentVector.EMPTY_NODE (array) 0))

(set! js/cljs.core.PersistentVector.fromArray
  (fn [xs ^boolean no-clone]
    (let [l (alength xs)
          xs (if no-clone xs (aclone xs))]
      (if (< l 32)
        (PersistentVector. nil l 5 js/cljs.core.PersistentVector.EMPTY_NODE xs nil)
        (let [node (.slice xs 0 32)
              v (PersistentVector. nil 32 5 js/cljs.core.PersistentVector.EMPTY_NODE node nil)]
...
(defn vec [coll]
  (-persistent!
   (reduce -conj!
           (-as-transient js/cljs.core.PersistentVector.EMPTY)
           coll)))

(defn vector [& args]
  (if (and (instance? IndexedSeq args) (zero? (.-i args)))
    (js/cljs.core.PersistentVector.fromArray (.-arr args) true)
    (vec args)))
...
(deftype ChunkedSeq [vec node i off meta ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll]
    (with-meta js/cljs.core.PersistentVector.EMPTY meta))
...
(deftype PersistentQueue [meta count front rear ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] js/cljs.core.PersistentQueue.EMPTY)
...
(defn- obj-map->hash-map [m k v]
  (let [ks  (.-keys m)
        ...
        mm  (meta m)]
    (loop [i   0
           out (transient js/cljs.core.PersistentHashMap.EMPTY)]
      (if (< i len)
        (let [k (aget ks i)]
...
(deftype ObjMap [meta keys strobj update-count ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta js/cljs.core.ObjMap.EMPTY meta))
  ...
  (-assoc [coll k v]
    (if ^boolean (goog/isString k)
        (if (or (> update-count js/cljs.core.ObjMap.HASHMAP_THRESHOLD)
                (>= (alength keys) js/cljs.core.ObjMap.HASHMAP_THRESHOLD))
...
(set! js/cljs.core.ObjMap.EMPTY (ObjMap. nil (array) (js-obj) 0 0))
...
(deftype PersistentArrayMap [meta cnt arr ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (-with-meta js/cljs.core.PersistentArrayMap.EMPTY meta))
  ...
  (-assoc [coll k v]
    (let [idx (array-map-index-of coll k)]
      (cond
        (== idx -1)
        (if (< cnt js/cljs.core.PersistentArrayMap.HASHMAP_THRESHOLD)
          (let [arr (array-map-extend-kv coll k v)]
            (PersistentArrayMap. meta (inc cnt) arr nil))
          (-> (into js/cljs.core.PersistentHashMap.EMPTY coll)
            (-assoc k v)
            (-with-meta meta)))
...
(set! js/cljs.core.PersistentArrayMap.EMPTY (PersistentArrayMap. nil 0 (array) nil))

(set! js/cljs.core.PersistentArrayMap.HASHMAP_THRESHOLD 8)
  ...
  (fn [arr ^boolean no-clone ^boolean no-check]
    (let [arr (if no-clone arr (aclone arr))] 
      (if no-check
        ...
        (let [len (alength arr)]
          (loop [i 0
                 ret (transient js/cljs.core.PersistentArrayMap.EMPTY)]
            (if (< i len)
              (recur (+ i 2)
  ...
  (-assoc! [tcoll key val]
    (if editable?
      (let [idx (array-map-index-of tcoll key)]
        (if (== idx -1)
          (if (<= (+ len 2) (* 2 js/cljs.core.PersistentArrayMap.HASHMAP_THRESHOLD))
            (do (set! len (+ len 2))
                (.push arr key)
                (.push arr val)
                tcoll)
...
(defn- array->transient-hash-map [len arr]
  (loop [out (transient js/cljs.core.PersistentHashMap.EMPTY)
         i   0]
    (if (< i len)
        ...
        (let [n (bit-count bitmap)]
          (if (>= n 16)
            (let [nodes (make-array 32)
                  jdx   (mask hash shift)]
              (aset nodes jdx (.inode-assoc js/cljs.core.BitmapIndexedNode.EMPTY (+ shift 5) hash key val added-leaf?))
              (loop [i 0 j 0]
                (if (< i 32)
                  (if (zero? (bit-and (bit-shift-right-zero-fill bitmap i) 1))
                    ...
                    (do (aset nodes i
                              (if-not (nil? (aget arr j))
                                (.inode-assoc js/cljs.core.BitmapIndexedNode.EMPTY
      ...
      (if (zero? (bit-and bitmap bit))
        (let [n (bit-count bitmap)]
          (cond
            ...
            (let [nodes (make-array 32)
                  jdx   (mask hash shift)]
              (aset nodes jdx (.inode-assoc! js/cljs.core.BitmapIndexedNode.EMPTY edit (+ shift 5) hash key val added-leaf?))
              (loop [i 0 j 0]
                (if (< i 32)
                  (if (zero? (bit-and (bit-shift-right-zero-fill bitmap i) 1))
                    (recur (inc i) j)
...
(set! js/cljs.core.BitmapIndexedNode.EMPTY (BitmapIndexedNode. nil 0 (make-array 0)))
...
(deftype ArrayNode [edit ^:mutable cnt ^:mutable arr]
  ...
  (inode-assoc [inode shift hash key val added-leaf?]
    (let [idx  (mask hash shift)
          node (aget arr idx)]
      (if (nil? node)
        (ArrayNode. nil (inc cnt) (clone-and-set arr idx (.inode-assoc js/cljs.core.BitmapIndexedNode.EMPTY (+ shift 5) hash key val added-leaf?)))
  ...
  (inode-assoc! [inode edit shift hash key val added-leaf?]
    (let [idx  (mask hash shift)
          node (aget arr idx)]
      (if (nil? node)
        (let [editable (edit-and-set inode edit idx (.inode-assoc! js/cljs.core.BitmapIndexedNode.EMPTY edit (+ shift 5) hash key val added-leaf?))]
     ...
     (let [key1hash (hash key1)]
       (if (== key1hash key2hash)
         ...
         (let [added-leaf? (Box. false)]
           (-> js/cljs.core.BitmapIndexedNode.EMPTY
...
(deftype PersistentHashMap [meta cnt root ^boolean has-nil? nil-val ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (-with-meta js/cljs.core.PersistentHashMap.EMPTY meta))
...
(set! js/cljs.core.PersistentHashMap.EMPTY (PersistentHashMap. nil 0 nil false nil 0))

(set! js/cljs.core.PersistentHashMap.fromArrays
  (fn [ks vs]
    (let [len (alength ks)]
      (loop [i 0 ^not-native out (transient js/cljs.core.PersistentHashMap.EMPTY)]
        (if (< i len)
...
(deftype PersistentTreeMap [comp tree cnt meta ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta js/cljs.core.PersistentTreeMap.EMPTY meta))
...
(set! js/cljs.core.PersistentTreeMap.EMPTY (PersistentTreeMap. compare nil 0 nil 0))

(defn hash-map
  "keyval => key val
  Returns a new hash map with supplied mappings."
  [& keyvals]
  (loop [in (seq keyvals), out (transient js/cljs.core.PersistentHashMap.EMPTY)]
    (if in
    ...
    (loop [kvs (seq keyvals)]
      (if kvs
        (do (.push ks (first kvs))
            ...
            (recur (nnext kvs)))
        (js/cljs.core.ObjMap.fromObject ks obj)))))

(defn sorted-map
  "keyval => key val
  Returns a new sorted map with supplied mappings."
  ([& keyvals]
     (loop [in (seq keyvals) out js/cljs.core.PersistentTreeMap.EMPTY]
       (if in
         (recur (nnext in) (assoc out (first in) (second in)))
         out))))
...
(deftype PersistentHashSet [meta hash-map ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta js/cljs.core.PersistentHashSet.EMPTY meta))
...
(set! js/cljs.core.PersistentHashSet.EMPTY
  (PersistentHashSet. nil js/cljs.core.PersistentArrayMap.EMPTY 0))

(set! js/cljs.core.PersistentHashSet.fromArray
  (fn [items ^boolean no-clone]
    (let [len (alength items)]
      (if (<= len js/cljs.core.PersistentArrayMap.HASHMAP_THRESHOLD)
        (let [arr (if no-clone items (aclone items))]
          (loop [i 0
                 out (transient js/cljs.core.PersistentArrayMap.EMPTY)]
            (if (< i len)
              (recur (inc i) (-assoc! out (aget items i) nil))
              (js/cljs.core.PersistentHashSet. nil (-persistent! out) nil))))
       (loop [i 0
              out (transient js/cljs.core.PersistentHashSet.EMPTY)]
         (if (< i len)
...
(deftype PersistentTreeSet [meta tree-map ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta js/cljs.core.PersistentTreeSet.EMPTY meta))
...
(set! js/cljs.core.PersistentTreeSet.EMPTY
  (PersistentTreeSet. nil js/cljs.core.PersistentTreeMap.EMPTY 0))
...
(defn sorted-set
  ...
  ([& keys]
   (reduce -conj js/cljs.core.PersistentTreeSet.EMPTY keys)))
...
(deftype Range [meta start end step ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [rng] (with-meta js/cljs.core.List.EMPTY meta))
...
(set! js/cljs.core.ExceptionInfo.prototype (js/Error.))
(set! (.-constructor js/cljs.core.ExceptionInfo.prototype) ExceptionInfo)
